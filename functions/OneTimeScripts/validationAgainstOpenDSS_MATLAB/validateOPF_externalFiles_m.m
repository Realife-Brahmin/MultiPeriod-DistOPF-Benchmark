...
fidMonitorStorage = fopen(fullfile(dssFolder, 'MonitorStorage.dss'), 'w');
fidLoadShape = fopen(fullfile(dssFolder, 'LoadShape.dss'), 'w');
fidLoadShapePSubsCost = fopen(fullfile(dssFolder, 'LoadShapePSubsCost.dss'), 'w');
...
strLoadShape = strcat('New Loadshape.LoadShape npts = ', num2str(T), ' interval = 1 mult = [', num2str(loadShape'), ']') %#ok
DSSText.Command = strLoadShape;
fprintf(fidLoadShape, '%s\n', strLoadShape);
...
if Batt_percent > 0
    ...
    for battBusNum = 1:nBatt
        ...
        fprintf(fidLoadShapeStorageControl, '%s\n', strLoadShapeSC);
