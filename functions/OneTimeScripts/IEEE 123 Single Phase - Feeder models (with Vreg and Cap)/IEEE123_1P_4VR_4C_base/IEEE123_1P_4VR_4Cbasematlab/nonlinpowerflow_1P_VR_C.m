...
PG = zeros(1,nb);              % rated active power of node                           % rated  DG active power
...
CVR_P = 0;                %% CVR factor for P = 0
CVR_Q = 0;                  %%% CVR factor for Q = 0
 ...
 beq(3*(nb-1)+1) = (tap(18)^2)*Vs;
%   Aeq(3*(nb-1)+1,Volttable(1)) = 1;
%  beq(3*(nb-1)+1) = (tap(17)^2)*Vs;
 end
Aeq;

Aeq(265,392) = -(tap(12)^2);
Aeq(283,406) = -(tap(21)^2);
Aeq(330,457) = -(tap(13)^2);

% Aeq(265,392) = -(tap(17)^2);
% Aeq(283,406) = -(tap(17)^2);
% Aeq(330,457) = -(tap(17)^2);
