function [Aeq, beq, lb_AreaAll, ub_AreaAll, x0, areaInfo] = LinEqualities(areaInfo, simInfo, v_parent_Area_1toT, varargin)
    ...
    % Input parser
    p = inputParser;
    % addParameter(p, 'delta_t', simInfo.delta_t, @isnumeric);
    % addParameter(p, 'etta_C', simInfo.etta_C, @isnumeric);
    % addParameter(p, 'etta_D', simInfo.etta_D, @isnumeric);
    ...
    P_L_Area_1toT = areaInfo.P_L_Area_1toT;

    % Q_L_Area = areaInfo.Q_L_Area;
    Q_L_Area_1toT = areaInfo.Q_L_Area_1toT;

    Q_C_Area = areaInfo.Q_C_Area;
    % P_der_Area = areaInfo.P_der_Area;
    P_der_Area_1toT = areaInfo.P_der_Area_1toT;
    ...
    % [areaInfo.fb_Area areaInfo.tb_Area]
     for j = 2 : N_Area
        ...
        beq(row) = Q_L_Area_1toT(j, :)' - Q_C_Area(j);
    ...
    for der_num = 1:nDER_Area
        ...
        % row
        % beq(row)
        beq(row) = beq(row) - P_der_Area_1toT(j, :)';
