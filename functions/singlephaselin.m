function [x_Area, Table_Area, indices_v_Full] = ...
    ...
    Area, N_Area, graphDFS_Area, graphDFS_Area_Table, R_Area_Matrix, X_Area_Matrix, P_L_Area, Q_L_Area, Q_C_Area, P_der_Area, ...
 ...
 % Default values for optional arguments
    ...
    validArgs = ["verbose", "CVR", "V_max", "V_min"];
    ...
    if ~isLeaf_Area
        myfprintf(verbose, "Area %d is not a leaf area, does have child areas.\n", Area);
        ...
        myfprintf(verbose, "Area %d does NOT have any child areas.\n", Area);
    end
    ...
    %     xIdx = length(x_Area) - N_Area + 1 - length(busesWithDERs_Area) + j;
    %     xIdx = 2*N_Area
    %     myfprintf(true, "%d\n", xIdx);
    %     % V2(Table_Area(j,2),1)= x_Area((end-N_Area+1-(size(busesWithDERs_Area,1)))+j);
    %     V2(Table_Area(j,2),1)= x_Area(xIdx);
    % 
    % end
