...
V(2,Tot_Area) = 0;
% Dec_Var(: ,Tot_Area) = 0;
Dec_Var = zeros(size(Se, 1)+1, Tot_Area);
% Loop:
...
while (c)
    ...
    for Area = 1:Tot_Area
        [v2, Ss(Area), S_all, mic_iter(iter+1,Area), Dec_Var_1, time_dist(iter+1,Area), R, T, nb] = ...
...
%         if (size(v2,1)< size (V,1))  % Ensuring the dimensions will match to store the voltages
        if lenv2 < lenV
            v2 = [v2; zeros(lenV - lenv2, 1)];
            Dec_Var_1 = [Dec_Var_1; zeros(lenV - lenv2, 1)];
            lenDec_Var = size(Dec_Var, 1);
            if lenV > lenDec_Var
                Dec_Var = [Dec_Var; zeros(lenV - lenDec_Var, 4)];
            end      
        elseif lenv2 > lenV
            V = [V; zeros(lenv2 - lenV, 4)];
            lenDec_Var = size(Dec_Var, 1);
            if lenv2 > lenDec_Var
                Dec_Var = [Dec_Var; zeros(lenv2 - lenDec_Var, 4)];
            end
%             V(end+1:size(v2,1),:) = 0;
